# bot/backtest.py  (MIN-HOLD + COOLDOWN + EOD SQUARE-OFF, TZ-SAFE)
...
# --- helper: normalize timestamps into market TZ (default Asia/Kolkata) ---
def _to_market_tz(ts: pd.Timestamp, market_tz: str) -> pd.Timestamp:
    """
    If ts is tz-aware -> convert to market_tz.
    If ts is naive    -> assume already in market time and return as is.
    """
    try:
        if ts.tz is None:
            return ts
...
def run_backtest(df_in: pd.DataFrame, cfg: Dict, use_block: str = "backtest_loose") -> Tuple[Dict, pd.DataFrame, pd.Series]:
    ...
    # --- merge plan from config (defaults + profile overrides) ---
    plan: Dict = {}
    ...
    # EOD square-off controls
    sess_end_str = str(plan.get("session_end", "15:20"))       # e.g. "15:20"
    try:
    ...
    for i, (ts, row) in enumerate(df.iterrows()):
        ...
        # current timestamp in market TZ (if index is tz-aware)
        ...
        if position == FLAT:
            ...
            # --- HARD GUARANTEE: if we ever carried overnight by mistake, exit at next morning's first bar ---
            # Compare by DATE in market tz
            last_entry_ts = trades[-1].entry_time
            last_entry_mkt = _to_market_tz(last_entry_ts, market_tz)
            if ts_mkt.date() > last_entry_mkt.date():
                # force exit on next day's first available bar at OPEN
            ...
            # --- Normal intraday EOD square-off: exit at/after session_end on same day (TZ-safe) ---
            ...
            # --- Standard exit logic (SL/TP) allowed only after min_hold bars ---
            exit_px = None
            reason = None
            if hold >= min_hold:
                if position == LONG:
            ...
            # --- Trailing stop after min_hold (ATR-based) ---
    ...
    sharpe = (rets.mean() / (rets.std() + 1e-9)) * np.sqrt(252 * 390) if rets.std() > 0 else 0.0  # 1m bars approx
...
def _plot_equity(eq: pd.Series, outdir: Path):
    ...
    fig.tight_layout()
    (outdir / "equity_curve.png").parent.mkdir(parents=True, exist_ok=True)
    ...
    fig2.tight_layout()
    fig2.savefig(outdir / "drawdown.png", dpi=120)
    plt.close(fig2)
