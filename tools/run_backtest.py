...
def map_interval(arg: str) -> str:
    ...
    return {
        "1m": "minute","3m":"3minute","5m":"5minute","10m":"10minute","15m":"15minute",
        "1minute":"minute","3minute":"3minute","5minute":"5minute","10minute":"10minute","15minute":"15minute",
    }.get(m, "5minute")
...
def main():
    ...
    if not idx_token:
        ...
        if u == "BANKNIFTY": idx_token = 260105
        elif u == "NIFTY":   idx_token = 256265
        else: raise SystemExit("Set intraday_options.index_token")
    ...
    if prices.empty: raise SystemExit("No candles fetched.")

    print("⚙️  Trade config:", {
        "order_qty": cfg.get("order_qty"),
        "capital_rs": cfg.get("capital_rs"),
        "reentry": (cfg.get("backtest",{}).get("reentry",{})),
        "guardrails": (cfg.get("backtest",{}).get("guardrails",{})),
        "exits": (cfg.get("backtest",{}).get("exits",{})),
    })
    ...
    try:
        meta = {
            ...
            "interval": args.interval, "n_rows": int(prices.shape[0]),
        ...
        print("ℹ️ latest.json write skipped:", e)
